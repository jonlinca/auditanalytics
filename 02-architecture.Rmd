...
For audit teams who have expertise in programming, RStudio Connect offers some of the best capabilities for publishing visualizations and analysis to your teams and internal stakeholders. With its git capability, it can also receive updates from a code repository, integrating tightly with a team's best practices of a code repository.
...
Audit teams historically had a bad reputation for 'bringing down the database'. While this used to be quite common, nowadays computing power is so accessible and cheap that the fear is generally unwarranted. However, great caution should still be taken, where you should avoid querying any production database that supports customers or staff directly, unless absolutely necessary. Other strategies are useful depending on the circumstances:

- Perform joins in the database, instead of downloading them individually and joining them on the desktop,
...
Certain online cloud software providers will make an Application Programming Interface (API) available for consumers. 

While APIs enable audit teams to retrieve data, it may add a layer of difficulty to obtaining data:

- APIs tend to be specific-use cases and provide a limited amount of data at one time.
- Data may not be in the format that you want - you are at the mercy what the API supplies.
...
- An API should be part of the requirements before signing an agreement with a provider. 
- Ask for access to the API, as well as any documentation.
- Generate a API key for usage, and treat it like a password.
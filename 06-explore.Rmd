...
For this section we will use the accounting database and validate our Journal Entry file prior to testing. We cover the importing of data in the prior chapter from a database:
...
- `gl_date` is numeric while `gl_date_char` is a character. We will talk about converting these dates to something interpretable in the next chapter.
...
- the code is difficult to read, as it is a long string variable names,
- is prone to errors as you have to type out each variable,
...
To make this code more reusable, we could make some changes. We notice that the activity columns are in the second column through the thirteenth position - we can assume that the 'position' of the columns will never change - second column will always be January, and the thirteenth will be December.
...
The newly calculated row summarization will return the total activity, and each item in each vector represents which row it belonged to. We can use the same TB account names and match it to the `tb_activty` object we made, and then create a new data frame with the account name and trial balance activity we calculated.
...
> Using the TB table (this becomes our LHS), create a column named "tb_activity", calculated as...

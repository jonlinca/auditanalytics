...
Lets say we were working on an audit to help determine if a transaction will be eventually charged against the correct account class. Specifically, lets see if we can predict if an a transaction is a fixed asset, based on the description field itself.
...
- To create a prediction, the need the fields used in training must also be the same words for the prediction. In our case, since we're using individual words, some additional work needs to be done to renormalize the words in the same format as our training dataset. For example, if we start seeing the word "Mac" instead of "Computer", we either need to drop the word "Mac" before running our prediction, or retrain our model with these new keywords. 

...
To create a prediction, we need to consider what we features we want to use. Lets go through a few of these columns to determine what to retain and remove:
...
- Some algorithms offer the ability to tune hyperparameters - that is, how to customize the algorithms ability to optimize itself. Tuning hyperparameters is so powerful, that even using a single model can yield significantly better results.
...
If you wanted to fully explore text mining, read up [Text Mining with R by Julia Silge & David Robinson](https://www.tidytextmining.com). In our example, we only explored the words themselves - there are additional branches of text mining that explore topic modelling, stemming, sentiment, among others approaches.
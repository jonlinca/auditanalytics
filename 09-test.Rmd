...
dupes_set1 <- invoices %>%
  ...
  mutate(dupes1_same_id_date_amt = row_number()) %>%
...
dupes_set2 <- invoices %>%
  ...
  filter(times_occured > 1) %>%
  ungroup() %>% # Remove the grouping aspect, which affects the ranking by row_number()
  mutate(dupes2_same_date_amt_diff_id = row_number()) %>%
  select(-times_occured)
...
dupes_set3 <- invoices %>%
  ...
  filter(times_occured > 1) %>%
  ungroup() %>% # Remove the grouping aspect, which affects the ranking by row_number()
  mutate(dupes3_same_date_amt_acct = row_number()) %>%
  select(-times_occured)
...
dupes_set4 <- invoices %>%
  ...
  filter(times_occured > 1) %>%
  ungroup() %>% # Remove the grouping aspect, which affects the ranking by row_number()
  mutate(dupes4_same_date_amt_acctcode_diff_id = row_number()) %>%
  select(-times_occured)

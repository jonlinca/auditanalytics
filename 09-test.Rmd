...
invoices %>%
  gt() # We'll hide this table-making code in future chunks
```
...
```{r, sss1, include = FALSE, message = FALSE, eval = FALSE}
dupes_set1 <- invoices %>%
...
decorate('sss1') %>%
  flair("group_by(invoice_id, date, amount)") %>%
  flair("summarize(times_occured = n())") %>%
  flair("filter(times_occured > 1)")  %>%
...
What is useful is then joining this information to your main dataset. This way, you get to maintain a library of invoices that meet multiple criteria. First off, lets keep keep all the duplicate keys we summarized on, plus a unique identifier so we can match the pairs:

```{r set1, include = FALSE, message = FALSE}
dupes_set1 <- invoices %>%
  group_by(invoice_id, date, amount) %>%
  summarize(times_occured = n()) %>%
  filter(times_occured > 1) %>%
  ungroup() %>% # Remove the grouping aspect, which affects the ranking by row_number()
  mutate(dupes1 = row_number()) %>%
  select(-times_occured)
```

```{r, echo = FALSE, message = FALSE}
decorate('set1') %>%
  flair('mutate(dupes1 = row_number())') %>%
  knit_print.with_flair()
```
...
This approach is unique but intuitive. By taking all possible 'same' combinations, we've effectively grouped any sort of duplicate `invoice_id` we may have already had. We ungroup and then resummarize without the `invoice_id`, which tells us how many times the `invoice_id` is different within the new group. 
...
Lastly, to derived the final list, its is effective to filter the results based on the `dupes` columns. 

Starting with the invoices we've identified, we'll first group by unique `system_id` we've created, allowing us to perform calculations on unique rows. To determine if any test did return something, we will take a `sum`, and return a TRUE (with a value of 1 if coerced). Then, we want to calculate this on multiple columns, specifically with the name starting with `dupes`, so we will use `across()` with `starts_with()`.

```{r, message = FALSE}
invoices %>%
  rowwise(system_id) %>%
  dplyr::mutate(dupes = sum(across(starts_with('dupes')) >= 1, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(dupes > 0) %>%
  gt()
```

This approach is robust. By maintaining our approach of having a main `invoice` dataset with all the matches, any new tests we add starting with the column name `dupes` will be included. For example, if we inserted a higher precision test of same invoice date, amount, and account code, we'll see our counts go up:
